...
class TerminoRealAposDataStatusRule extends RegraHealthCheckBase
{
    ...
    {
        return 'Término real registrado após a data de status';
    }

    public function descricao(int $quantidade): string
    {
        return "Foram encontradas {$quantidade} atividade(s) com término real posterior à data de status do cronograma.";
    }

    public function impacto(): string
    {
        return 'Uma conclusão no futuro (em relação à data de status) distorce a medição do período e os indicadores de desempenho — normalmente indica apontamento no período errado.';
    }
    ...
    protected function combina(TarefaImportada $tarefa, PlanoImportacao $plano): bool
    {
        return $tarefa->realTermino !== null
            ...
            && $tarefa->realTermino->gt($plano->dataStatus);
    }
}
